...
It’s like a “virtual host” in Apache.
...
- `server_name example.com;` — Server responds to requests for example.com.
- `location / { ... }` — For any URL starting with `/`, serve files from `/var/www/html`.
- `root /var/www/html;` — The directory containing static files.
- `index index.html index.htm;` — If a directory is requested (like `/`), serve `index.html` or `index.htm` if found.
...
- `http://example.com/` → loads index.html.
- `http://example.com/static/style.css` → loads CSS file.
...
5. Open browser and visit `http://example.com` (or server IP).
   You should see your Python app’s response.
...
- `weight=3` means the first server gets 3x more traffic.
- `backup` means the server is used only if others fail.
...
Backend servers don’t need SSL
